...
def build_loss(loss_conf):
    """
    build loss
    loss_conf: {loss_type1: weight | [weight,args_dict], ...}
    """
    loss_func = partial(weighted_loss, loss_conf=loss_conf)
    return loss_func
...
def weighted_loss(output, target, loss_conf):
    """
    weighted multi-loss
    loss_conf: {loss_type1: weight | [weight,args_dict], ...}
    """
    ...
    for k, v in loss_conf.items():
        if isinstance(v, (float, int)):
            ...
            raise ValueError(
                f"the Key({k})'s Value {v} in Dict(loss_conf) should be scalar(weight) | list[weight, args] ")
...
if __name__ == "__main__":
    ...
    loss_conf = {'l1_loss': 0.5, 'mse_loss': 0.5}

    loss_v = weighted_loss(output, target, loss_conf)
    print('loss: ', loss_v)
